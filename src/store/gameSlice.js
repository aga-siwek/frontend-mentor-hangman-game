  ...
  reducers: {
    ...
    startGame: (state, action) => {
      ...
      const randomPassword =
        passwordsData["categories"][state.category][randomIndex];
...
const isWordComplete = (word, selectedLetters) => {
  ...
  const isComplete = Array.from(letterSet).every((letter) =>
...
listenerMiddleware.startListening({
  ...
  effect: (action, listenerApi) => {
    ...
    if (
      isLetterInWord(state.selectPassword, lastLetter) &&
      isWordComplete(state.selectPassword, state.selectedLetters)
    ) {
